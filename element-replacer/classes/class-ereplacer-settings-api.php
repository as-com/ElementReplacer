...
/**
 * ElementReplacer Settings API Class
 *
 * A settings API (wrapping the WordPress Settings API).
 *
 * @package WordPress
 * @subpackage ereplacer_Settings_API
 * @category Settings
 * @author Andrew Sun
 * @since 1.0.0
 */
class ereplacer_Settings_API {
	public $token;
	public $name;
	private $_settings;
	public $sections;
	...
	public function init_sections() {
		...
		_e( 'Override init_sections() in your class.', '' );
	} // End init_sections()
	...
	public function init_fields() {
		...
		_e( 'Override init_fields() in your class.', '' );
	} // End init_fields()
	...
	public function settings_screen() {
		global $ereplacer;
		do_action( $ereplacer->token . '_before_settings' );
		...
		do_action( $ereplacer->token . '_after_settings' );
		submit_button();
	} // End settings_screen()
	...
	protected function add_error ( $key, $data ) {
		if ( isset( $data['error_message'] ) ) {
			...
			$message = sprintf( __( '%s is a required field', '' ), $data['name'] );
		}
	...
	protected function parse_errors() {
		if ( count ( $this->_errors ) > 0 ) {
			...
			$message = sprintf( __( '%s settings updated', '' ), $this->name );
